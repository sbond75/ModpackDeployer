...
1. To compile the updater into an application that others can execute *and* to update your local copy at the path configured within the updater GUI, run `deployAndUpdate.bat` on Windows (bash is required since it is invoked within the file) or `bash ./deployAndUpdate.bat` on macOS or Linux (this file is so simple that it works on all three operating systems). You can also perform individual steps from this process using the steps under [Usage](##Usage).
...
3. Extract the MultiMC zip file that downloaded in the previous step.
4. Open the folder that was extracted.
5. Run MultiMC.exe (or whatever the program name is). If you get a message saying "Windows Defender SmartScreen prevented an unrecognized app from starting. Running this app might put your PC at risk.", you can click "More info" and then "Run anyway" to bypass it.
6. Add your Minecraft account at the top-right.
7. Click "Add Instance."
8. Click "Import from zip."
9. Click "Browse" and choose the modpack which was downloaded in step 2.
10. Click "OK" and wait for it to extract and install the modpack.
11. Double-click the new modpack icon that appears inside MultiMC, and the game will start up.
